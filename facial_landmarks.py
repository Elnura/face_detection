...
Dudak = Enum('Dudak', 'buyukkabarik kucukkabarik ustkalinaltince ustincealtkalin ustsivri ustyuvarlak ustduz ince siradan')
...
def facial_landmarks(image_base64__, b_day, b_month, b_year):
    image_byte_ = base64.b64decode(image_base64__)
    b_date = datetime(b_year, b_month, b_day)  # year month day
    with open(temp_image_file, 'wb') as f_output:
        f_output.write(image_byte_)
    ...
    faces = Faces(rects)
    faces_result = faces.assignFaceDistances(gray)
    relations_result = assign_relations(faces_result, b_date)
